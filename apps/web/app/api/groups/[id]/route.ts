...
export async function PATCH(
  ...
  if (isGroupUpdateData(requestBody)) {
    //verify that the given data object implements the updateGroupById interface
    const prismaResponse: PrismaResponse<Group> = await updateGroupById(
      Number(id),
      requestBody
    );
    if (prismaResponse.status === 200 && prismaResponse.data) {
