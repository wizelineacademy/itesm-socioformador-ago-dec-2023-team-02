...
export async function GET(
  ...
  // The function returns a Promise that resolves to a NextResponse object
  ...
  if (result.status !== 200) {
    ...
    return new NextResponse(result.message || "An error occurred", {
...
export async function PATCH(
  ...
  const body: UpdatedInfo = await request.json();
  ...
  const result = await updateConversationById(idConversation, body);
