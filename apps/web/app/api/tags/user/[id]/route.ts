...
/**
 * GET request handler, fetches all tags belonging to the specified user. 
 * @param request - The request object.
 * @param params - The request's parameters, which contain the ID of the user whose tags will be fetched. 
...
export async function GET(request: Request, {params: {id}}: {params: {id: number}}): Promise<NextResponse> {
    const prismaResponse: PrismaResponse<Tag[]> = await getAllTagsByUserID(Number(id))
