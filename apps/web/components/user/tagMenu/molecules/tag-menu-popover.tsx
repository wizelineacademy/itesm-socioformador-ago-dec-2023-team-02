...
export default function TagMenuPopover({children, tags, initialSelectedTags, placement, onPopoverClose}: TagMenuPopoverProps): JSX.Element {
    ...
    const handleOpenChange: (isOpen: boolean) => void = (isOpen) => {setPopoverIsOpen(isOpen)}
    ...
    return (
        <Popover backdrop="opaque" isOpen={popoverIsOpen} onClose={handlePopoverClose} onOpenChange={handleOpenChange} placement={placement} showArrow>
            <PopoverTrigger>
                {children}
            </PopoverTrigger>
            <PopoverContent> 
                <div className="flex flex-col justify-center itesm-center gap-4 w-96 p-4">
                    ...
                    <div className="flex flex-row gap-2 justify-center items-center">
                        ...
                        <ButtonWithIcon icon={<IoCheckmarkSharp color="white"/>} isDisabled={false} onPress={handleSaveButtonPress} style="bg-sky-700" text="Confirm"/>
                    </div>
                </div>
            </PopoverContent>
        </Popover>
