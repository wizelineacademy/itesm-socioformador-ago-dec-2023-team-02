...
export function ConversationCard({conversation, conversationsDispatch, isSelected, onClick}: ConversationCardProps): JSX.Element {
    ...
    const titleWhenNotEditing: JSX.Element = <p className="text-xs text-white whitespace-nowrap overflow-scroll">{title}</p>
    const titleWhenEditing: JSX.Element = (
        <input className="text-xs text-white rounded-sm pr-10" onChange={handleTitleChange} onClick={handleTitleClick} onKeyDown={handleTitleKeydown} type="text" value={title}/>
    )
    ...
    return (
        <button className="w-full group relative p-0" onClick={onClick} ref={cardContainerRef} type="button">
            <Card className={`max-w-[200px] py-2 pl-2 pr-0 border-none rounded-md shadow-none hover:bg-white hover:bg-opacity-20 ${cardBackgroundColor}`} radius="none">
