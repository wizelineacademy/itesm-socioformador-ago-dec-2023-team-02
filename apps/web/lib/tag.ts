...
import {
  isValidTag,
  normalizeTagCreateData,
  normalizeTagUpdateData,
  type TagCreateData,
  type TagUpdateData,
...
export async function getTag(idTag: number): Promise<PrismaResponse<Tag>> {
  try {
    ...
    return tag === null
      ? { status: 404, message: "No tag was found" }
      : { status: 200, data: tag };
  } catch (error: any) {
 ...
 * holds all tags of the selected user.
 */
export async function getAllTagsByUserAuthID(
  idAuth: string
): Promise<PrismaResponse<Tag[]>> {
  try {
    ...
    if (!user) {
      return { status: 400, message: "Invalid user Auth ID given." };
    }
 ...
 * holds all tags of the selected user.
 */
export async function getAllSidebarTagsByUserID(
  idUser: number
): Promise<PrismaResponse<Tag[]>> {
  try {
    if ((await prisma.user.findUnique({ where: { id: idUser } })) === null) {
 ...
 * holds all tags of the selected user.
 */
export async function getAllSidebarTagsByUserAuthID(
  idAuth: string
): Promise<PrismaResponse<Tag[]>> {
  try {
    //get user based on id auth0
 ...
 */
export async function getAllTagsByConversationID(
...
export async function createTag(
  ...
  if (tagData.idUser <= 0 || !isValidTag(normalizedTagData)) {
    return {
      status: 400,
      message: "Invalid user ID, name, or color value given.",
    };
  }
  ...
  try {
    ...
    if (
      (await prisma.user.findUnique({ where: { id: tagData.idUser } })) === null
    ) {
      return { status: 400, message: "Invalid user ID given." };
    }
 ...
 * @returns A promise that resolves to a PrismaResponse object containing either the created tag or an error message.
 */
export async function createTagbyAuthId(
  idAuth: string,
  tagData: TagCreateData
): Promise<PrismaResponse<Tag>> {
  //get user based on id auth0
